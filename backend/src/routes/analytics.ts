...
import { listInsights, regenerateInsights } from '../services/insight.service';
...
function triggerInsightRegen(userId: string) {
...
analyticsRouter.get('/dashboard', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const range = (req.query.range as DateRangeType) || 'current_week';
    ...
    // Fetch lesson counts
    const lessonCounts = await query<{ source: string; count: string }>(
...
analyticsRouter.get('/tasks', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.userId;
    if (!userId) throw new UnauthorizedError();
...
analyticsRouter.post('/tasks', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.userId;
    if (!userId) throw new UnauthorizedError();
...
analyticsRouter.put('/tasks/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.userId;
    if (!userId) throw new UnauthorizedError();
...
analyticsRouter.delete('/tasks/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.userId;
    if (!userId) throw new UnauthorizedError();
...
analyticsRouter.post('/focus-sessions', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.userId;
    if (!userId) throw new UnauthorizedError();
...
analyticsRouter.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { studentName, activityName, score, attendanceStatus, engagementScore, date } =
        req.body;
