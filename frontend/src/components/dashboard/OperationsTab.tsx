...
const ROUTINE_ICONS: Record<string, typeof UtensilsCrossed> = {
...
export function OperationsTab() {
  const [activePanel, setActivePanel] = useState<'enquiries' | 'routines'>('enquiries');
  ...
  // Routine State
  const [routines, setRoutines] = useState<DaycareRoutine[]>([]);
  const [routinesLoading, setRoutinesLoading] = useState(true);
  const [routineChild, setRoutineChild] = useState('');
  const [routineType, setRoutineType] = useState<'meal' | 'nap' | 'diaper' | 'activity'>('meal');
  ...
  const handleCreateEnquiry = async (e: FormEvent) => {
    e.preventDefault();
  ...
  const handleStatusUpdate = async (
    id: string,
    newStatus: 'pending' | 'contacted' | 'admitted' | 'rejected',
  ) => {
    try {
                ...
                {enquiries.map((enq) => (
                  ...
                  >
                    ...
                    <div className="flex gap-1.5 pt-1">
                      {(['pending', 'contacted', 'admitted', 'rejected'] as const)
            ...
            <form onSubmit={handleCreateRoutine} className="space-y-3 text-xs font-bold">
              ...
              <div className="space-y-1">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {(['meal', 'nap', 'diaper', 'activity'] as const).map((rt) => {
