...
export function QuickControls({ analytics }: QuickControlsProps) {
  const [activeTab, setActiveTab] = useState<'hours' | 'tasks' | 'students'>('hours');

  // Log Hours Form State
  const [hours, setHours] = useState('2');
  const [activityType, setActivityType] = useState<
    'planning' | 'teaching' | 'grading' | 'focus_session'
  >('planning');
  ...
  const handleLogStudent = async (e: React.FormEvent) => {
    e.preventDefault();
